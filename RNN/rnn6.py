...
def train():
    ...
    model.add(LSTM(2**6,return_sequences=True,input_shape=[12,1]))
    model.add(MaxPooling1D(2))
    #model.add(LSTM(n_units//2,return_sequences=True,input_shape=[12,1]))
    #model.add(Conv1D(2 ** 5, 2, activation="relu", padding='same'))
    ...
    for i in range(1, 8):
        ...
        result.save('Classified_images_2/5_class_'+str(i)+'.tiff')
